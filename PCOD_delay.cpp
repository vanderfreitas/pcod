...
// ------------------- MODEL ---------------------------
void PCOD_delay::rotation_center_delay(double *state, bool store){
...
double PCOD_delay::uk_circular_symmetric_paley_all_to_all_delay(double theta, int id){
...
void PCOD_delay::derivs(double y[],double df[], double interp_t, bool store){

	int index=0, i;

	if(delay){
		// Claculate the rotation center of the particles
		rotation_center_delay(y, store);
		...
		for(i=1; i<=NE; i+=3){
			...
			df[i]=uk_circular_symmetric_paley_all_to_all_delay(y[i], index);
			df[i+1]=cos(y[i]);
			df[i+2]=sin(y[i]);
...
void PCOD_delay::polint(double xa[], double ya[], int n, double x, double *y, double *dy){
...
void PCOD_delay::retrieve_past_values_via_interpolation(double interp_t, int num_points){
...
PCOD_delay::PCOD_delay(int N_, double M_, double omega0_, double tau_){
	int i,j;	
	...
	particles = new particle_delay[N]; // (particle_ *)malloc((size_t) ((N_)*sizeof(particle_)));
...
void PCOD_delay::step_forward(){
	...
	derivs(y,df, t_minus_tau, true); // Delay at (t-tau)
	...
	retrieve_past_values_via_interpolation(t_minus_tau + h_over_2, 5);
	derivs(y,df, t_minus_tau + h_over_2, false); // Delay at (t-tau+h/2)
	...
	derivs(y,df, t_minus_tau+h, false); // Delay at (t-tau+h)
